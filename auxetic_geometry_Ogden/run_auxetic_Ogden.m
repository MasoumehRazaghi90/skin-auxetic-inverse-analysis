...
materialType='Ogden'; % HGO, Ogden, neo-Hookean

isIsotropic   = ismember(materialType, {'neo-Hookean','Ogden'});
isAnisotropic = strcmp(materialType,'HGO');
...
c1_ini=3.96;
m1_ini=14.354;
c2_ini = 0.192;    % initial guess for second term
m2_ini = 14.482;   % initial guess (negative m)
k_ini=100*c1_ini;
...
if isAnisotropic
    angles_deg = 0:15:90;   % Only GOH needs fiber orientation sweep
else
    angles_deg = 0;         % Isotropic materials: single run
end
...
for ang = angles_deg
    ...
    % Plot material directions (ONLY for anisotropic materials)
if isAnisotropic
    cFigure; hold on;
    ...
    % Visualizing material directions (ONLY for anisotropic materials)
if isAnisotropic
    [VE]=patchCentre(E,V);
    ...
    % -> ElementData
    %MeshData section
% -> ElementData (ONLY for anisotropic materials)
if isAnisotropic
    ...
    %Output section
    % -> log file
    logNamePrefix = sprintf('%s_ang_%03d', febioFebFileNamePart, ang);
